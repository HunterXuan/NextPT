...
VALUES
    ('tracker', 'announce_interval', '1800', '客户端心跳汇报间隔（秒）', NOW(), NOW()),
    ('tracker', 'announce_min_interval', '900', '客户端心跳最小汇报间隔（秒）', NOW(), NOW()),
    ('tracker', 'announce_url', '"http://127.0.0.1/api/tracker/announce"', 'Tracker 宣告地址', NOW(), NOW()),
    ('tracker', 'bonus_T0', '8.0', '魔力值衰减参数 T0', NOW(), NOW()),
    ('tracker', 'bonus_N0', '7.0', '魔力值做种人数拥挤惩罚基数 N0', NOW(), NOW()),
    ('tracker', 'bonus_B0', '100.0', '魔力值每小时体积收益硬上限 B0', NOW(), NOW()),
    ('tracker', 'bonus_L', '300.0', '魔力值收益收敛平滑参数 L', NOW(), NOW()),
    ('tracker', 'bonus_base', '0.4', '每个达标种子的基础每小时奖励', NOW(), NOW()),
    ('iam', 'default_role', '"user"', '新注册用户默认角色标识', NOW(), NOW()),
    ('iam', 'default_register_role', '1', '新注册用户默认角色ID', NOW(), NOW()),
    ('iam', 'register_enabled', 'true', '是否开放全站注册', NOW(), NOW()),
    ('catalog', 'torrent_source', '"[NextPT]"', '私有种子 source 标识', NOW(), NOW())
ON DUPLICATE KEY UPDATE
    `value` = VALUES(`value`),
