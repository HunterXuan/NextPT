...
> **定位**：这是一个轻量的管理组收件箱，不引入工单分类、关联目标、申诉流程或复杂会话。用户发送一条消息，管理组回复或关闭，用户在自己的列表中查看处理结果。

* **用户侧**
  * 用户只能查看自己发出的消息。
  * 新消息创建后状态为 `pending`。
  * 管理组回复后状态为 `answered`，关闭后状态为 `closed`。
  * 管理组回复或关闭时，通过 SiteMessage 通知发信用户。
* **管理侧**
  * 管理员按状态、用户 ID分页查看所有消息。
  * `answered` 需要填写回复内容；`closed` 可以直接关闭，也可以附带最后说明。
  * 已关闭消息不再修改，处理动作记录到 SiteAudit。
...
- `PATCH /api/admin/mod/staff-messages/{id}`
  - 权限：`admin:mod/staff-message:*`
  - 设置 `status` 为 `answered` 或 `closed`，并可保存回复内容。
